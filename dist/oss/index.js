...
var MemoryVectorStore = class {
  ...
  filterVector(vector, filters) {
    ...
    return Object.entries(filters).every(
      ([key, value]) => Array.isArray(value) && Array.isArray(vector.payload[key]) ? value.every((item) => vector.payload[key].includes(item)) : vector.payload[key] === value
    );
  }
...
// src/oss/src/prompts/categorization.ts
...
function buildCategorizationInput(userMessage, categories) {
...
var categorizationResponseFormat = {
  type: "json_schema",
  json_schema: {
    name: "categories",
    schema: {
...
// src/oss/src/vector_stores/open_search.ts
...
var OpenSearchVectorStore = class {
  constructor(config) {
    this.userId = null;
    var _a2;
    let node;
    ...
    const auth = config.user && config.password ? { username: config.user, password: config.password } : void 0;
    const defaults = {
      maxRetries: 3,
      requestTimeout: 3e4
    };
    ...
    if ("awsV4signerOptions" in config) {
      awsV4signerOptions = (0, import_aws.AwsSigv4Signer)({
        region: "ap-northeast-1",
        service: "es",
        // 'aoss' for OpenSearch Serverless
        getCredentials: () => {
          const credentialsProvider = (0, import_credential_provider_node.defaultProvider)();
    ...
    this.client = new import_opensearch.Client({
      ...
      auth,
      ssl: {
        rejectUnauthorized: (_a2 = config.verifyCerts) != null ? _a2 : true
      },
      ...awsV4signerOptions,
  ...
  async createCol(name, vectorSize) {
    const indexExists = await this.client.indices.exists({ index: name }).catch(() => {
      return { body: false };
    });
  ...
  async search(query, limit = 5, filters) {
    ...
    const filterClauses = [];
    if (filters) {
      for (const key of ["userId", "runId", "agentId"]) {
        const value = filters[key];
        if (value) {
          filterClauses.push({ term: { [`payload.${key}.keyword`]: value } });
        }
      }
      for (const arrayKey of ["categories"]) {
    ...
    const queryBody = {
      size: limit * 2,
      query: filterClauses.length ? { bool: { must: knnQuery, filter: filterClauses } } : knnQuery
    };
    ...
    return hits.map((hit) => {
      ...
      return {
        id: hit._source.id,
        score: hit._score,
        payload: (_a2 = hit._source.payload) != null ? _a2 : {}
      };
    });
  }
  async get(vectorId) {
    ...
    const hit = hits[0];
    return {
      id: (_a2 = hit._source) == null ? void 0 : _a2.id,
      score: 1,
      payload: (_c = (_b = hit._source) == null ? void 0 : _b.payload) != null ? _c : {}
    };
  }
  async update(vectorId, vector, payload) {
    const searchQuery = { query: { term: { id: vectorId } }, size: 1 };
  ...
  async list(filters, limit) {
    ...
    const queryBody = filterClauses.length ? { query: { bool: { filter: filterClauses } } } : { query: { match_all: {} } };
    if (limit) {
    ...
    const hits = response.body.hits.hits;
    const total = typeof response.body.hits.total === "number" ? response.body.hits.total : typeof response.body.hits.total === "object" && ((_a2 = response.body.hits.total) == null ? void 0 : _a2.value) ? hits.length : 0;
    const results = hits.map((hit) => {
      var _a3;
      return {
        id: hit._source.id,
        score: 1,
        payload: (_a3 = hit._source.payload) != null ? _a3 : {}
      };
    });
...
var Memory = class _Memory {
  constructor(config = {}) {
    ...
    this.customCategories = /* @__PURE__ */ new Map();
